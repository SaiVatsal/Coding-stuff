...
BAWSAQ_TICKERS: List[Dict[str, Any]] = [
    {
        "ticker": "FRUT",
...
def get_ticker_by_bawsaq_symbol(ticker: Optional[str]) -> Optional[Dict[str, Any]]:
    if not ticker:
        return None
    upper = ticker.upper().strip()
    for t in BAWSAQ_TICKERS:
        if t["ticker"] == upper:
            return t
    return None


def get_ticker_by_real_symbol(symbol: Optional[str]) -> Optional[Dict[str, Any]]:
    if not symbol:
        return None
    upper = symbol.upper().strip()
    for t in BAWSAQ_TICKERS:
        if t["real_ticker"] == upper:
            return t
    return None
