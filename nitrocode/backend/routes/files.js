...
export function getWorkspaceRoot(req) {
  ...
  // Fallback to process.cwd() or parent directory
...
router.post('/save', async (req, res) => {
  try {
    ...
    // Notify files changed to dynamic watchers
...
// GET /api/files/search - Grep search
    ...
    async function searchDir(dir) {
      ...
      for (const file of files) {
        ...
        if (stat.isDirectory()) {
          ...
          if (['.js', '.jsx', '.ts', '.tsx', '.json', '.html', '.css', '.md', '.py', '.go', '.rs', '.txt', '.sh', '.env'].includes(ext)) {
